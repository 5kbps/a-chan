	...
	function CheckBannedHash() {
		...
		if (isset($_FILES['imagefile'])) {
			if ($_FILES['imagefile']['name'] != '') {
				...
				if (count($results) > 0) {
						$bans_class->BanUser($_SERVER['REMOTE_ADDR'], null, 'SERVER', '1', $results[0]['bantime'], '', 'Posting a banned file.<br />' . $results[0]['description'], 0, 0, 1);
						$bans_class->BanCheck($_SERVER['REMOTE_ADDR'], $board_class->board['name'], false);
						die();
				}
			}
		}
		...
		foreach ($bad_ords as $bad_ord) {
			if ($ords_name != '') {
				if (in_array($bad_ord, $ords_name)) {
					exitWithErrorPage(_gettext('Недопустимые символы в посте.'));
				}
			}
			if ($ords_email != '') {
		...
		foreach ($badlinks as $badlink) {
			if (stripos($_POST['message'], $badlink) !== false) {
				...
				if ($_SERVER['REMOTE_ADDR'] === '127.0.0.1') {
					session_start();
					$bans_class->BanUser(null, session_id(), 'board.php', 1, 3600, '', _gettext('Спамлист!') . ' (' . $badlink . ')', $_POST['message'], 0, 3);
				} else {
					$bans_class->BanUser($_SERVER['REMOTE_ADDR'], null, 'board.php', 1, 3600, '', _gettext('Спамлист!') . ' (' . $badlink . ')', $_POST['message']);
				}
				exitWithErrorPage(sprintf(_gettext('Спамлист! ( %s )'), $badlink));
			}
		}
	}
}
