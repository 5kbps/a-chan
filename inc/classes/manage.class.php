...
class Manage {
	...
	function LoginForm() {
		...
		if (file_exists(KU_ROOTDIR . 'inc/pages/manage_login.html')) {
			...
			$tpl_page .= 'static login page: 404';
		}
	}
	...
	*/
	...
	function announcements() {
		...
		$tpl_page .= '<a href="/manage_page.php?action=globallog"><h3>Global Log</h3></a>';
		$entries = 0;
	...
	function posting_rates() {
		...
		$posts_total = 0;
		$tpl_page .= '<h2>'. _gettext('Posting rates (past '.$period.'s.)') . '</h2><br />';
		...
		if (count($results) > 0) {
			...
			foreach ($results as $line) {
				$rows_threads = $tc_db->GetOne("SELECT HIGH_PRIORITY count(id) FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $line['id'] . " AND `parentid` = 0 AND `timestamp` >= " . (time() - $period));
				$rows_replies = $tc_db->GetOne("SELECT HIGH_PRIORITY count(id) FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $line['id'] . " AND `parentid` != 0 AND `timestamp` >= " . (time() - $period));
				$rows_posts = $rows_threads + $rows_replies;
				$posts_total += $rows_posts;
			...
			$tpl_page .= '</table>';
			$tpl_page .= "Total: $posts_total, speed=" . ($posts_total / ($period/3600)). '/h.';
		} else {
	...
	*/
	...
	function globallog() {
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		require_once KU_ROOTDIR . 'inc/classes/globallog.class.php';
		$globallog = new GlobalLog();
		$globallog->addEntry('Globallog opened!', "globallog_opened");
		echo $globallog->getHTML($globallog->selectBetweenTimeStamps());
	}
	function globallog_deleteall(){
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		$tc_db->Execute("DELETE FROM `" . KU_DBPREFIX . "globallog` WHERE `timestamp` > 0");
		$this->globallog();
	}
	function globallog_multidelete(){
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		$fst = $_GET['fst'];
		$lst = $_GET['lst'];
		$tc_db->Execute("DELETE FROM `" . KU_DBPREFIX . "globallog` WHERE `id` > ".$tc_db->qstr($fst-1)." AND `id` < ".$tc_db->qstr($lst+1));
		$this->globallog();
	}
		...
		if (isset($_FILES['imagefile'])) {
			...
			foreach($proxies as $proxy) {
				if (preg_match('/.[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+.*/', $proxy)) {
					$proxy = trim($proxy);
					$ips++;
					if ($bans_class->BanUser(preg_replace('/:.*/', '', $proxy), null, 'SERVER', 1, 0, '', 'IP from proxylist automatically banned', '', 0, 0, 1, true)) {
						$successful++;
					}
				}
			}
			...
			if (count($results) > 0) {
				if (!empty($confirm)) {
					if (removeBoard($board_dir)) {
						...
						$tc_db->Execute("DELETE FROM `" . KU_DBPREFIX . "antiwipe` WHERE `id` = '" . $board_id . "'");
	...
	function embeds() {
		...
		function movethread() {
			...
			if (isset($_POST['id']) && isset($_POST['board_from']) && isset($_POST['board_to'])) {
				...
				$results = $tc_db->GetAll("SELECT `id`, `message` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " .$board_from_id. " AND `parentid` = " . $id . " ORDER BY `id` ASC");
				$textRep = array(); // <- our replacements would be here
				$repl1 =  '<a href=\\"/'.$board_from.'/res/'.$id_unq.'.html#'.$id_unq .'\\" onclick=\\"return highlight(\\\''.$id_unq .'\\\', true);\\" class=\\"ref|'.$board_from.'|'.$id_unq.'|'.$id_unq .'\\">&gt;&gt;'.$id_unq.'</a>';
				$repl2 =  '<a href=\\"/'.$board_to  .'/res/'.$new_id.'.html#'.$new_id .'\\" onclick=\\"return highlight(\\\''.$new_id .'\\\', true);\\" class=\\"ref|'.$board_to  .'|'.$new_id.'|'.$new_id .'\\">&gt;&gt;'.$new_id.'</a>';
				$textRep[$repl1] = $repl2;
				foreach ($results as $line) {
					if (isset($_POST['mf'])) {
					...
					// $board_from -> $board_to -- board dir
					// $id -> $new_id -- thread
					// $line['id'] -> $insert['id'] -- post id
					$repl1 =  '<a href=\\"/'.$board_from.'/res/'.$id_unq.'.html#'.$line['id'].'\\" onclick=\\"return highlight(\\\''.$line['id'].'\\\', true);\\" class=\\"ref|'.$board_from.'|'.$id_unq.'|'.$line['id'].'\\">&gt;&gt;'.$line['id'].'</a>';
					$repl2 =  '<a href=\\"/'.$board_to  .'/res/'.$new_id.'.html#'.$insert_id .'\\" onclick=\\"return highlight(\\\''.$insert_id .'\\\', true);\\" class=\\"ref|'.$board_to  .'|'.$new_id.'|'.$insert_id .'\\">&gt;&gt;'.$insert_id .'</a>';
					$textRep[$repl1] = $repl2;
					// modifying message
					$old_message = $line['message'];
					$new_message = strtr($old_message, $textRep);
					$new_message = $tc_db->qstr($new_message,true);
					$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "posts` SET `id` = " . $insert_id . ", `boardid` = " .$board_to_id. ", `message` = ". $new_message ." WHERE `boardid` = " .$board_from_id. " AND `id` = " . $line['id']);
					// </MOD>
					// old : $tc_db->Execute("UPDATE `" . KU_DBPREFIX . "posts` SET `id` = " . $insert_id . ", `boardid` = " .$board_to_id. " WHERE `boardid` = " .$board_from_id. " AND `id` = " . $line['id']);
		...
		function ipsearch() {
			...
			if (isset($_GET['ip']) && !empty($_GET['board'])) {
				...
				if (count($results) > 0) {
					$tpl_page .= '<h3>Total: '.count($results).'</h3><br />'. "\n";
					foreach ($results as $line) {
		...
		function sessionsearch(){
			...
			if(!empty($_GET['session'])){
				...
				$limit = 10;
				$query = "SELECT  * FROM `" . KU_DBPREFIX . "posts` WHERE `IS_DELETED` = 0 AND `session` = ".$tc_db->qstr($session)." ORDER BY `timestamp` DESC";
				$results = $tc_db->GetAll($query);
				$tpl_page .= '<p style="font-size: 1.5em;">'. _gettext('Results for') .': <strong>'. $session . '</strong></p>';
				$count = 0;
				require_once 'inc/func/calculations.php';
				$tmp_page = '';
				foreach ($results as $line) {
					$line['boardname'] = $tc_db->GetOne("SELECT HIGH_PRIORITY `name` FROM `" . KU_DBPREFIX . "boards` WHERE `id` = " . $tc_db->qstr($line['boardid']));
					//$line['boardname'] = boardid_to_dir($line['boardid']);
					$tmp_page .= '<span style="font-size: 1.5em;">'. $count . '.</span> <span style="font-size: 1.3em;">'. _gettext('Board') .': /'. $line['boardname'] . '/, <a href="'.KU_BOARDSPATH . '/'. $line['boardname'] . '/res/';
					if ($line['parentid'] == 0) {
						$tmp_page .= $line['id'] . '.html">';
					} else {
						$tmp_page .= $line['parentid'] . '.html#'. $line['id'] . '">';
					}

					if ($line['parentid'] == 0) {
						$tmp_page .= _gettext('Thread') .' #'. $line['id'];
					} else {
						$tmp_page .= _gettext('Thread') .' #'. $line['parentid'] . ', Post #'. $line['id'];
					}
					$tmp_page .= '</a></span>';

					$line['message'] = stripslashes($line['message']);
					$tmp_page .= '<fieldset>'. $line['message'] . '</fieldset><br />';
					$count++;
				}
				$tpl_page .= "TOTAL: $count<br>";
				$tpl_page .= $tmp_page;
				if($_GET['nocss']){
					echo $tpl_page;
					die();
				...
				<p>Session ID:</p> <input type="text" name="session">
				</form>';
			}
		}
		...
		function rebuildall() {
			...
			require_once KU_ROOTDIR . 'inc/classes/staticpages.class.php';
			$menu_class = new StaticPages();
			$menu_class->GenerateFAQ();
			$tpl_page .=  _gettext('Regenerated FAQ') .'<br />';

			$tpl_page .= sprintf(_gettext('Rebuild complete. Took <strong>%d</strong> seconds.'), time() - $time_start);
			$tpl_page .= '<br><a href="/manage_page.php?action=rebuildall" id="afocus">Rebuild</a><script>document.getElementById("afocus").focus()</script>';
		...
		*/
		...
		function rebuild_antiwipe() {
			require_once KU_ROOTDIR . 'inc/classes/antiwipe.class.php';
			global $tc_db, $tpl_page, $board_class;
			#!
			$this->AdministratorsOnly();
			$boards = $tc_db->GetAll("SELECT HIGH_PRIORITY `id`, `name` FROM `" . KU_DBPREFIX . "boards`");
			foreach ($boards as $line){
				$board_id   = $line['id'];
				$board_name = $line['name'];
				// check if already in table
				$tc_db->Execute("INSERT INTO `".KU_DBPREFIX."antiwipe` ( `id` , `name` ) VALUES ( ".$tc_db->qstr($board_id)." , ".$tc_db->qstr($board_name).")");
			}
		}
		function antiwipe() {
			require_once KU_ROOTDIR . 'inc/classes/antiwipe.class.php';
			...
			#!
			$this->AdministratorsOnly();
			$tpl_page .= "<h2>" . _gettext("Antiwipe") . "</h2>";
			$tpl_log = '';
			$fields  = ['id', 'name', 'timefilter', 'timefiltervalue', 'timethreadfilter', 'timethreadfiltervalue',
			'wipefilter', 'logall', 'jscheck', 'pow', 'powvalue', 'powafter', 'confirmation', 'confirmationafter','panicindex','panicindextimeout', 'fullauto'];
			$fields_checkboxes = ['timefilter', 'timethreadfilter', 'wipefilter', 'logall', 'jscheck', 'pow','confirmation', 'fullauto'];
			$fields_inputs     = ['timefiltervalue', 'timethreadfiltervalue', 'powvalue', 'confirmationafter', 'powafter','panicindex', 'panicindextimeout'];
			$query = "SELECT HIGH_PRIORITY `" . join($fields,'` , `') . "` FROM `" . KU_DBPREFIX . "antiwipe` ORDER BY `name`";
			$results = $tc_db->getAll($query);
			if(!$results){
				$boards = $tc_db->GetAll("SELECT HIGH_PRIORITY `id`, `name` FROM `" . KU_DBPREFIX . "boards`");
				foreach ($boards as $line){
			...
			$tpl_page .= '<form action="manage_page.php?action=antiwipe" method="post"><table border=1>';
			$tpl_page .= '<tr><td>'.join($fields, '</td><td style="font-size:13px;">').'</td></tr>';
			$updates = [];
			foreach ($results as $result) {
				...
				$tpl_page .= "<tr>";
				// update panic index
				$antiwipe_class = new Antiwipe($b_id);
				foreach ($fields as $i => $field){
					if($_SERVER['REQUEST_METHOD']=='POST'){
						$updates[$b_name][$field] = $result[$i];
						$tpl_log .= "<br> $b_name [$field] = $result[$i]";
						if(isset($_POST[$field . '_' . $b_name])){
							$newvalue = $_POST[$field . '_' . $b_name]=="on"?1:$_POST[$field . '_' . $b_name];
						}else{
							$newvalue = 0;
						}
						//$newvalue = !isset($newvalue)?0:$newvalue;
						if(isset($newvalue)){
							if($i > 1 && $newvalue != $result[$i]){
								$tpl_log .= "<br><b style='color:green'>Updating: $field" . "_" . "$b_name from " . $result[$i]  . " to $newvalue </b>";
								$tc_db->Execute("UPDATE `".KU_DBPREFIX."antiwipe` SET `$field`='$newvalue' WHERE `name`=".$tc_db->qstr($b_name));
								$result[$i] = $newvalue;
							}
						}else{
							if($i > 1 && $result[$i]){
								$tc_db->Execute("UPDATE `".KU_DBPREFIX."antiwipe` SET `$field`='$newvalue' WHERE `name`=".$tc_db->qstr($b_name));
								$tpl_log .= "<br><b style='color:red'>Updating: $field" . "_" . "$b_name from " . $result[$i]  . " to $newvalue </b>";
								$result[$i] = $newvalue;
							}
						}
					}
					...
					switch ($i) {
						case 1:
						$tpl_page .= '<a href="/' . $r . '">/'.$r . '/</a>';
						break;
						default:
						$fname = $field.'_'.$b_name;
						if(in_array($field, $fields_checkboxes)){
							$tpl_page .= '<input type="checkbox" name="'.$fname.'" id="'.$fname.'" '.($r?'checked':'').'>';
						}elseif(in_array($field, $fields_inputs)){
							$tpl_page .= '<input style="width:50px;" name="'.$fname.'" id="'.$fname.'" value="'.$r.'">';
						}
						break;
					}
					$tpl_page .= "</td>";
			...
			$tpl_page .= '</table><input type="submit" value="'._gettext("Submit").'"></form>';
			//$tpl_page .= $tpl_log;
		...
		/* get_board_id */
		function get_board_id($boarddir) {
			global $tc_db;
			$boardid = $tc_db->GetOne("SELECT HIGH_PRIORITY `id` FROM `" . KU_DBPREFIX . "boards` WHERE `name` = " . $tc_db->qstr($boarddir) . "");
			if ($boardid) {
				return $boardid;
			}else{
				die('Can\'t get board id');
			}
		}
		/* review status */
		function addreviewstatus() {
			global $tc_db, $tpl_page, $board_class;
			$this->AdministratorsOnly();
			// if you want to publish this code, add validation & emptiness check
			if ($_REQUEST['boarddir'] && $_REQUEST['id'] && $_REQUEST['status_html']) { 
				// $_REQUEST['postid'];
				$old_subject = $this->get_post_subject($_REQUEST['boarddir'], $_REQUEST['id']);
				$new_subject = $this->add_status($old_subject, $_REQUEST['status_html']);
				$this->set_post_subject($_REQUEST['boarddir'], $_REQUEST['id'], $new_subject);
				echo($new_subject);
				die();
			} else {
				http_response_code(500);
				die('Check input paramters');
			}
		}
		function get_post_subject($boarddir, $id) {
			global $tc_db, $tpl_page, $board_class;
			$boardid = $this->get_board_id($boarddir);
			return $tc_db->GetOne('SELECT HIGH_PRIORITY `subject` from `posts` where `boardid` = ' . $tc_db->qstr($boardid) . ' AND `id` = '.$tc_db->qstr($id));
		}
		function set_post_subject($boarddir, $id, $new_subject) {
			global $tc_db, $tpl_page, $board_class;
			$boardid = $this->get_board_id($boarddir);
			$tc_db->Execute('UPDATE `posts` SET `subject` = ' . $tc_db->qstr($new_subject) .' where `boardid` = ' . $tc_db->qstr($boardid) . ' AND `id` = '.$tc_db->qstr($id));
			$board_class = new Board($boarddir);
			$board_class->RegenerateThreads($id);
			$board_class->RegeneratePages();
		}
		function split_subject($subject) {
			return explode($this->get_subject_separator(), $subject);
		}
		function is_status_in_subject($subject, $status) {
			return strpos($subject, $status) !== false;
		}
		function add_status($subject, $status) {
			if ($this->is_status_in_subject($subject, $status)) {
				return $subject;
			} else {
				return $status . $this->get_subject_separator() . $subject;
			}
		}
		function remove_status($subject, $status) {
			$arr - $this->split_subject($subject);
			$r = [];
			...
			return implode($this->get_subject_separator(), $r);
		}
		function get_subject_separator() {
			return '<span class="rs-separator"></span>';
		}
		/* soft deletion */
		function softdelposts($multidel=false) {
			global $tc_db, $tpl_page, $board_class;
						...
						if ($_POST['delthreadid'] > 0) {
							...
							if (count($results) > 0) {
								if (isset($_POST['fileonly'])) {
									...
									$post_class->SoftDelete($mod_name='moderator', $harddel_empty_thread = false);
		...
		*/
		...
		function bans() {
			...
			if (((isset($_POST['ip']) || isset($_POST['multiban']) || isset($_POST['session'])) && isset($_POST['seconds']) && (!empty($_POST['session']) || !empty($_POST['ip']) || (empty($_POST['ip']) && !empty($_POST['multiban'])))) || $instantban) {
				if ($_POST['seconds'] >= 0 || $instantban) {
					...
					$ban_type = ($_POST['type'] <= 3 && $_POST['type'] >= 0) ? $_POST['type'] : 0;
					$ban_ip = ($instantban) ? $ban_ip : $_POST['ip'];
					$ban_session = $_POST['session'];
					if($ban_type == 3){
						//$ban_ip = null;//'255.255.255.255';
					}
					...
					foreach ($ban_ips as $ban_ip) {
						...
						if ($bans_class->BanUser($ban_ip, $ban_session, $_SESSION['manageusername'], $ban_globalban, $ban_duration, $ban_boards, $ban_reason, $ban_note, $ban_appealat, $ban_type, $ban_allowread)) {
							$regenerated = array();
			...
			$tpl_page .= '<br />';
			$tpl_page .= '<label for="session">Session id:</label>';
			$tpl_page .= '<input type="text" name="session" value="'.htmlspecialchars($ban_session).'">';
			$tpl_page .= '<br />
			...
			<label for="type">'. _gettext('Type') . ':</label>
			<select name="type" id="type"><option value="0">'. _gettext('Single IP') . '</option><option value="1">'. _gettext('IP Range') . '</option><option value="2">'. _gettext('Whitelist') . '</option><option value="3" '.(!empty($ban_session)?'selected':'') .'>Session</option></select>
			...
			<input type="checkbox" name="banfromall" id="banfromall" checked/><br /><hr /><br />' .
			...
			<script src="/static/js/min/bans.min.js"></script>
			<a href="#" onclick="setRule(1);return false;">ATT</a>&nbsp;
			<a href="#" onclick="setRule(2);return false;">SPAM</a>&nbsp;
			<a href="#" onclick="setRule(3);return false;">CP</a>&nbsp;
			<a href="#" onclick="setRule(4);return false;">POL</a>&nbsp;
			<a href="#" onclick="setRule(5);return false;">WIPE</a>&nbsp;
			</div><br />
			...
			for ($i = 3; $i >= 0; $i--) {
				switch ($i) {
					case 3:
					$tpl_page .= '<strong>'. _gettext('Session bans') . ':</strong><br />';
					break;
					case 2:
				...
				if (count($results) > 0) {
					$tpl_page .= '<table border="1" width="100%"><tr>';
					if($i != 3){
						$tpl_page .= '<th>'. (($i == 1) ? _gettext('IP Range') : _gettext('IP Address')). '</th>';
					}else{
						$tpl_page .= '<th>'._gettext('Session').'</th>';
					}
					$tpl_page .= '<th>'. _gettext('Boards') . '</th><th>'. _gettext('Reason') . '</th><th>'. _gettext('Staff Note') . '</th><th>'. _gettext('Date added') . '</th><th>'. _gettext('Expires/Expired') . '</th><th>'. _gettext('Added By') . '</th><th>&nbsp;</th></tr>';
					foreach ($results as $line) {
						$tpl_page .= '<tr>';
						if($i != 3){
							$tpl_page .= '<td><a href="?action=bans&ip='. md5_decrypt($line['ip'], KU_RANDOMSEED) . '">'. md5_decrypt($line['ip'], KU_RANDOMSEED) . '</a></td>';
						}else{
							$tpl_page .= '<td>'.htmlspecialchars($line['session']).'</td>';
						}
						if ($line['globalban'] == 1) {
							$tpl_page .= '<td><strong>'. _gettext('All boards') . '</strong></td>';
						} elseif (!empty($line['boards'])) {
							$tpl_page .= '<td><strong>/'. implode('/</strong>, <strong>/', explode('|', $line['boards'])) . '/</strong></td>&nbsp;';
						}
						$tpl_page .= '</td><td>';
						...
						$tpl_page .= '</td><td>'. $line['by'] . '</td><td>[<a href="manage_page.php?action=bans&delban='. $line['id'] . '">'. _gettext('Delete') .'</a>]</td>';
						$tpl_page .= '</tr>';
					}
					$tpl_page .= '</table>';
					if ($hiddenbans > 0) {
		...
		function editpost() {
			global $tc_db, $tpl_page, $board_class;
			$this->AdministratorsOnly();
			if(isset($_REQUEST['boarddir']) && isset($_REQUEST['postid'])){
				$boardid = $tc_db->GetOne("SELECT `id` FROM `" . KU_DBPREFIX . "boards` WHERE `name` = " . $tc_db->qstr($_REQUEST['boarddir']));
				if(isset($_REQUEST['message'])){
					$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "posts` SET `message` = " . $tc_db->qstr($_REQUEST['message']) . " WHERE `boardid` = " . $tc_db->qstr($boardid) . " AND `id` = " . $tc_db->qstr($_REQUEST['postid']));
					$board_class = new Board($_REQUEST['boarddir']);
					$board_class->RegenerateAll();
					$tpl_page .= "Edited!";
					$tpl_page .= '<br /><br /><meta http-equiv="refresh" content="1;url='. KU_BOARDSPATH . '/'. $_REQUEST['boarddir'] . '/"><a href="'. KU_BOARDSPATH . '/'. $_REQUEST['boarddir'] . '/">'. _gettext('Redirecting') . '</a> back to board...';
				}else{
					$post_text = $tc_db->GetOne("SELECT `message` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $tc_db->qstr($boardid) . " AND `id` = " . $tc_db->qstr($_REQUEST['postid']));
					$tpl_page .= '<form action="?action=editpost" method="post">
					<input name="postid" value="' . $_REQUEST['postid'] . '">
					<input name="boarddir" value="' . $_REQUEST['boarddir'] . '">
					<textarea name="message" style="width:100%; height:500px;">' . htmlspecialchars($post_text) . '</textarea><br>
					<input type="submit" value="'. _gettext('Edit post') .'" />
					</form><br />';
				}
			}else{
				$tpl_page .= '<form action="manage_page.php?action=editpost" method="post">
				<input type="hidden" name="token" value="' . $_SESSION['token'] . '" />
				<label for="boarddir">'. _gettext('Board') .':</label>' .
				$this->MakeBoardListDropdown('boarddir', $this->BoardList($_SESSION['manageusername'])) .
				'<br />
				<label for="postid">'. _gettext('Post') .':</label>
				<input type="text" name="postid" /><br />
				<input type="submit" value="'. _gettext('Edit post') .'" />
				</form>
				<br /><hr />';
		...
		*/
		...
		function addtospamlist() {
			...
			if (!empty($_REQUEST['domains'])) {
				$oldcontent = file_get_contents($spam);
				$newcontent = $oldcontent . $_REQUEST['domains'];
				// remove empty lilnes
				...
				$newcontent = implode("\n", $arr);
				file_put_contents($spam, $newcontent);
			}
			$this->spam();
		}
		...
		function getips() {
			...
			if (count($results) > 0) {
				...
				$query = '';
				$postids = explode(",", $_GET['ids']);
				foreach($postids as $i => $postid){
					if(is_numeric($postid)){
						$query .= '`id` = ' . $tc_db->qstr($postid);
						if($i != count($postids) - 1){
							$query .= ' OR ';
						}
					}
				}
				$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `id`, `parentid`, `ip`, `session` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $tc_db->qstr($results[0]['id']) . " AND " . $query);
				$output = array();
				foreach ($results as $key => $result) {
					$output[$result['id']] = array(
						'ip' => md5_decrypt($result['ip'], KU_RANDOMSEED),
