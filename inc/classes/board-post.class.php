  ...
  function RegeneratePages() {
    ...
    if ($this->board['enablecatalog'] == 1 && ($this->board['type'] == 0 || $this->board['type'] == 2)) {
      ...
      if ($numresults > 0) {
        ...
        $maxrows = max(1, (($numresults - ($numresults % $cols)) / $cols));
        foreach ($results as $line) {
          // table
          if($col == $cols){
            $col = 0;
          ...
          // post
          $thread_url = KU_BOARDSFOLDER . $this->board['name'] . '/res/' . $line['id'] . '.html';
          ...
          $catalog_page .= '<td class="catalog-td">' .
          '<div class="reply-like catalog-post">' .
          '<a href="' . $thread_url.'"';
          if ($line['subject'] != '') {
          ...
          if ($line['file'] != '' && $line['file'] != 'removed') {
            if (in_array($line['file_type'], ['gif','jpg','png','webm'])) {
              $file_path = getCLBoardPath($this->board['name'], $this->board['loadbalanceurl_formatted'], $this->archive_dir);
              $catalog_page .= '<img class="catalog-thumb" src="' . $file_path . '/thumb/' . $line['file'] . (($line['file_type'] === 'webm') ? 'webm.jpg' : ('s.'. $line['file_type']))  . '" alt="' . $line['id'] . '" border="0" />';
            } else {
              $catalog_page .= 'Файл';
            }
          } elseif ($line['file'] == 'removed') {
            $catalog_page .= '(Файл удалён)';
          } else {
            $catalog_page .= '(Нет файла)';
          }
          $catalog_page .= '</a>';
          $catalog_page .= '<small class="catalog-replies-count">' . $replies . '</small>';
          $message = wordwrap(strip_tags($line['message']), 80, "<br />\n", true);
          $message = explode("\n", $message);
          $message = array_slice($message, 0, 4);
          $message =  implode('', $message);
          $catalog_page .= ' '.$message;
          $catalog_page .= '</div>';
          $catalog_page .= '</td>';
          // table
          if($col == $cols){
...
class Post extends Board {
  ...
  function getMessage() {
    global $tc_db;
    $message = $tc_db->GetOne("SELECT `message` FROM `".KU_DBPREFIX."posts` WHERE `boardid` = ".$tc_db->qstr($this->board['id'])." AND `id` = ".$tc_db->qstr($this->post['id']));
    return $message;
  }
  function SoftDelete($mod_name = 0, $harddel_empty_thread=true) {
    global $tc_db;
    if(!$mod_name)
    $default_mod_name = "it's author";

    if($this->post['isthread']){
      // how many posts in the thread?
      $count_in_thread = $tc_db->GetOne("SELECT COUNT(*) FROM `".KU_DBPREFIX."posts` where `boardid` = '" . $this->board['id'] . "' AND `parentid` = ".$tc_db->qstr($this->post['id']));
      if(!$count_in_thread && $harddel_empty_thread){
        // fallback to Delete if 0
        return $this->Delete();
      }
    }
    $del_message = '<span class="post-deleted"><span class="gt">Post was deleted by</span> <span class="gt">'.htmlspecialchars($mod_name).'</span><input class="raw-deletion-timestamp" type="hidden" value="'.time().'" disabled></span>';
    $tc_db->Execute("UPDATE `".KU_DBPREFIX."posts` SET `message` = ".$tc_db->qstr($del_message)." WHERE `boardid` = '" . $this->board['id'] . "' AND `id` = ".$tc_db->qstr($this->post['id']));
    clearPostCache($this->post['id'], $this->board['name']);
    return true;
  }
  ...
  function Insert($parentid, $name, $tripcode, $email, $subject, $message, $filename, $file_original, $filetype, $file_md5, $image_w, $image_h, $filesize, $thumb_w, $thumb_h, $password, $timestamp, $bumped, $ip, $posterauthority, $tag, $stickied, $locked, $boardid) {
    ...
    $query = "INSERT INTO `".KU_DBPREFIX."posts` ( `parentid` , `boardid`, `name` , `tripcode` , `email` , `subject` , `message` , `file` , `file_original`, `file_type` , `file_md5` , `image_w` , `image_h` , `file_size` , `file_size_formatted` , `thumb_w` , `thumb_h` , `password` , `timestamp` , `bumped` , `ip` , `session`, `ipmd5` , `posterauthority` , `tag` , `stickied` , `locked` ) VALUES ( ".$tc_db->qstr($parentid).", ".$tc_db->qstr($boardid).", ".$tc_db->qstr($name).", ".$tc_db->qstr($tripcode).", ".$tc_db->qstr($email).", ".$tc_db->qstr($subject).", ".$tc_db->qstr($message).", ".$tc_db->qstr($filename).", ".$tc_db->qstr($file_original).", ".$tc_db->qstr($filetype).", ".$tc_db->qstr($file_md5).", ".$tc_db->qstr(intval($image_w)).", ".$tc_db->qstr(intval($image_h)).", ".$tc_db->qstr($filesize).", ".$tc_db->qstr(ConvertBytes($filesize)).", ".$tc_db->qstr($thumb_w).", ".$tc_db->qstr($thumb_h).", ".$tc_db->qstr($password).", ".$tc_db->qstr($timestamp).", ".$tc_db->qstr($bumped).", ".$tc_db->qstr(md5_encrypt($ip, KU_RANDOMSEED)).", ". $tc_db->qstr(session_id()) ." ,'".md5($ip)."', ".$tc_db->qstr($posterauthority).", ".$tc_db->qstr($tag).", ".$tc_db->qstr($stickied).", ".$tc_db->qstr($locked)." )";
    $tc_db->Execute($query);
    $id = $tc_db->Insert_Id();
