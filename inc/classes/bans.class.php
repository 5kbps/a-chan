...
class Bans {
	...
	function BanCheck($ip, $board = '', $force_display = false, $session = null) {
		global $tc_db;
		...
		$bans = Array();
		$results = $tc_db->GetAll("SELECT * FROM `".KU_DBPREFIX."banlist` WHERE (((`type` = 3 AND `session` = ".$tc_db->qstr($session).") OR (`type` = '0' AND ( `ipmd5` = '" . md5($ip) . "' OR `ipmd5` = '". md5($_COOKIE['tc_previousip']) . "' )) OR `type` = '1')) AND (`expired` = 0)" );
		if (count($results)>0) {
			foreach($results AS $line) {
				if($line['type'] == 3 || (($line['type'] == 1) && strpos($ip, md5_decrypt($line['ip'], KU_RANDOMSEED)) === 0) || $line['type'] == 0) {
	...
	function BanUser($ip, $session, $modname, $globalban, $duration, $boards, $reason, $staffnote, $appealat=0, $type=0, $allowread=1, $proxyban=false) {
		global $tc_db;
		...
		if($type == 3){
			$ip = null;// '255.255.255.255';
			//die('session ban in bans.class.php!');
		}
		if ($duration>0) {
			$ban_globalban = '0';
		} else {
		...
		$tc_db->Execute("INSERT INTO `".KU_DBPREFIX."banlist` ( `ip` , `session`, `ipmd5` , `type` , `allowread` , `globalban` , `boards` , `by` , `at` , `until` , `reason`, `staffnote`, `appealat` ) VALUES ( ".$tc_db->qstr(md5_encrypt($ip, KU_RANDOMSEED)) . " , " .$tc_db->qstr($session). " ," .$tc_db->qstr(md5($ip))." , ".intval($type)." , ".intval($allowread)." , ".intval($globalban)." , ".$tc_db->qstr($boards)." , ".$tc_db->qstr($modname)." , ".time()." , ".intval($ban_until)." , ".$tc_db->qstr($reason)." , ".$tc_db->qstr($staffnote).", ".intval($appealat)." ) ");
