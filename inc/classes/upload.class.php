						...
						if ($filetype_forcethumb == 0) {
							...
							/* Otherwise, use this script alone */
								...
								if($this->file_type == '.webm'){
									$this->file_name = time() . mt_rand(1, 99);
									$shell = "ffmpeg -y -i ".escapeshellarg($_FILES['imagefile']['tmp_name'])." -r 1 -vf scale=200:-1 -frames 1 ".escapeshellarg(KU_BOARDSDIR . $board_class->board['name'] . '/thumb/' . $this->file_name . 'webm.jpg')." 2>&1";
									$output = shell_exec($shell);
									$this->file_thumb_location = KU_BOARDSDIR . $board_class->board['name'] . '/thumb/' . $this->file_name . 'webm.jpg';
								}
