...
function formatLongMessage($message, $board, $threadid, $page) {
	$output = '';
	if ((strlen($message) > 10000 || count(explode('<br />', $message)) > 50) && $page) {
		...
		'	' . sprintf(_gettext('Пост слишком длинный. Нажмите %sздесь%s чтобы увидеть его целиком.'), '<a href="' . KU_BOARDSFOLDER . $board . '/res/' . $threadid . '.html">', '</a>') . "\n" .
		'</div>' . "\n";
	} else {
		$output .= $message . "\n";
	}
