...
function exitWithErrorForm($errormsg, $extended = '') {
	...
	echo $dwoo->get(KU_TEMPLATEDIR . '/img_error_form.tpl', $dwoo_data);
	die();
}

function ExitWithConfirmationForm($confirmation_settings, $check_js=false, $pow=false, $pow_value=0){
	global $dwoo, $dwoo_data;
	if (!isset($dwoo)) {
		require_once KU_ROOTDIR . 'lib/dwoo.php';
	}
	...
	$dwoo_data->assign('postdata', $postdata);
	$dwoo_data->assign('settings', $confirmation_settings);
	echo $dwoo->get(KU_TEMPLATEDIR . '/img_post_confirmation.tpl', $dwoo_data);
	die();
}
