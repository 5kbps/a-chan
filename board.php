...
require KU_ROOTDIR . 'inc/classes/antiwipe.class.php';
...
$globallog->addEntry(json_encode($_POST, JSON_UNESCAPED_UNICODE), "posting_attempt");
// }}}
...
if (isset($_POST['board'])) {
	...
	if (!empty($board_name)) {
		...
		$globallog->addEntry(json_encode($_POST), "redirect_main");
		do_redirect('/');
	}
} else {
	$globallog->addEntry(json_encode($_POST), "redirect_main");
...
$bans_class->BanCheck($_SERVER['REMOTE_ADDR'], $board_class->board['name'], false, session_id());
// }}}
...
if ($posting_class->CheckValidPost($is_oekaki)) {
	...
	$markup_wakabamark = isset($_POST['markup_wakabamark']);
	$markup_bbcode = isset($_POST['markup_bbcode']);
	$markup_other = isset($_POST['markup_other']);
	$imagefile_name = isset($_FILES['imagefile']) ? $_FILES['imagefile']['name'] : '';
	// read configuration
	$antiwipe_class = new AntiWipe($board_class->board['id'], $_POST);
	// Antiwipe check
	// by 5kbps
	$antiwipe_class->CheckReplyTime();
	$antiwipe_class->checkPOWorCaptcha();
	$antiwipe_class->CheckConfirmation();

	// confirmation settings
	if($imagefile_name != ''){
		$antiwipe_class->results['filename'] = $_FILES['imagefile']['tmp_name'];
	}else{
		$antiwipe_class->results['filename'] = '';
	}
	if(!$post_isreply){
		$antiwipe_class->TimeThreadFilter();
	}else{
		$antiwipe_class->TimeFilter();
	}
	// check all and exit if something is missed
	$antiwipe_class->CheckAll();
	...
	if (isNormalUser($user_authority)) {
		...
		if ($thread_locked == 1) {
			...
			exitWithErrorPage(_gettext('Извините, этот тред закрыт.'));
		}
		$post_message = $parse_class->ParsePost($_POST['message'], $board_class->board['name'], $board_class->board['type'], $thread_replyto, $board_class->board['id'], $ispage=false, $markup_wakabamark=$markup_wakabamark, $markup_bbcode=$markup_bbcode, $markup_other=$markup_other);
	...
	if ($post_isreply) {
		...
		if ($imagefile_name == '' && !$is_oekaki && ((!isset($_POST['nofile'])&&$board_class->board['enablenofile']==1) || $board_class->board['enablenofile']==0) && ($board_class->board['type'] == 0 || $board_class->board['type'] == 2 || $board_class->board['type'] == 3)) {
			if ($imagefile_name == '') {
				...
				exitWithErrorPage(_gettext('Для создания нового треда необходимо прикрепить файл.'));
				//}
			}
		}
	}
	...
	if (isset($_POST['nofile'])&&$board_class->board['enablenofile']==1) {
		if ($post_message == '') {
			exitWithErrorPage('Введите сообщение чтобы постить без файла.');
		}
	}

	if ($board_class->board['type'] == 1 && !$post_isreply && $post_subject == '') {
		exitWithErrorPage('Тема необходима для создания треда.');
		//
	}
	...
	if ($board_class->board['locked'] == 0 || ($user_authority > 0 && $user_authority != 3)) {
		...
		if ((file_exists(KU_BOARDSDIR . $board_class->board['name'] . '/src/' . $upload_class->file_name . $upload_class->file_type) && file_exists(KU_BOARDSDIR . $board_class->board['name'] . '/thumb/' . $upload_class->file_name . 's' . $upload_class->file_type)) || ($file_is_special && file_exists(KU_BOARDSDIR . $board_class->board['name'] . '/src/' . $upload_class->file_name . $upload_class->file_type)) || $post_fileused == false) {
			...
			exitWithErrorPage(_gettext('Невозможно скопировать загруженный файл.'));
		}
		...
		if ($thread_replyto != '0' && strtolower($_POST['em']) != 'sage' && unistr_to_ords($_POST['em']) != array(19979, 12370)) {
			...
			if ($thread_replies <= $board_class->board['maxreplies']) {
				...
				$globallog->addEntry($board_class->board['name'] . "/" . $thread_replyto, "bumped");
			}
		}
		...
		exitWithErrorPage('Этот раздел закрыт для постинга.');
	}
				...
				if ((md5($_POST['postpassword']) == $post_class->post['password'])|| $ismod) {
					if (isset($_POST['fileonly'])) {
						if ($post_class->post['file'] != '' && $post_class->post['file'] != 'removed') {
							...
							echo 'В вашем посте нет файла.' . '<br />';
						}
					} else {
						$oldmessage = $post_class->getMessage();
						if ($post_class->SoftDelete($ismod?"moderator":"it's author")) {
							$post_class->DeleteFile();
							if ($post_class->post_parentid != '0') {
								$board_class->RegenerateThreads($post_class->post['parentid']);
							}
							$board_class->RegeneratePages();
							$globallog->addEntry('Deleted: ' . $board_class->board['name'] . '/' . $post_class->post['id'] .'<br>' . $oldmessage, "post_deletion");
							echo 'Пост удален.<br />';
						} else {
							$globallog->addEntry('Deletion failed ', "post_deletion");
							echo 'Произошла ошибка при попытке удалить пост.' . '<br />';
						}
					}
				} else {
					$globallog->addEntry($board_class->board['name'] . '/' . $post_class->post['id'], "post_deletion_attempt");
					echo 'Невозможно удалить пост.' . '<br />';
				}
			} else {
				do_redirect( '/' . $board_class->board['name'] . '/');
			}
		}
	}
	do_redirect('/' . $board_class->board['name'] . '/');
...
if ($_POST['redirectto'] == 'thread') {
	if ($thread_replyto == "0") {
		do_redirect( '/' . $board_class->board['name'] . '/res/' . $post_id . '.html', true, $imagefile_name);
	} else {
		do_redirect('/' . $board_class->board['name'] . '/res/' . $thread_replyto . '.html', true, $imagefile_name);
	}
} else {
	if ($post_isreply) {
		do_redirect( '/' . $board_class->board['name'] . '/', true, $imagefile_name);
	} else {
		do_redirect( '/' . $board_class->board['name'] . '/res/' . $post_id . '.html', true, $imagefile_name);
	}
}
